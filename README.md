...
Line coverage is measured by `cargo llvm-cov` over every suite that runs
unprivileged (unit, property, integration and end-to-end) with all
features; CI fails below 90 % of lines. The tests and coverage badges
above are generated by that job on every push to `main` and stored on the
