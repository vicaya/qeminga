...
**Coverage:** the reply to `guest-fsfreeze-freeze-list` is the number of distinct superblocks, among those the requested mount points name, on which this operation's `FIFREEZE` succeeded, and nothing else (C-12, AC17). A requested mount point names exactly one thing: the superblock the pathname leads to *now*, decided by the mount graph of `/proc/self/mountinfo` (mount ids and parents: the mount at that mount point that nothing is stacked on, standing on ancestors that are themselves visible), never by the order of the table, since a mount moved over a newer one keeps its earlier row. It never names a superblock whose mount point is hidden, by a mount over it or over an ancestor (that superblock is named by the mount points it is still visible at, if any), so one requested name selects at most one superblock. The freeze then opens a selected superblock on the requested name and on nothing else, and the opened directory is verified to be on that superblock: a name that leads elsewhere fails the operation rather than being rescued by an alias, so the coverage a count certifies is the coverage of the names the controller gave. Aliases serve the thaw, which reaches and recovers a superblock through every mount point it has, hidden ones included (§4.2 above). A requested path outside the plan, a target the kernel does not freeze (`EOPNOTSUPP`) and a superblock another freezer already holds (`EBUSY`, not this operation's freeze under the single-freezer assumption) are all left out of the count, and two names of one superblock (a bind alias) count it once. A planned target that cannot be reached at all (none of its mount points opens on its superblock) is not a lower count but a failed operation: the completed targets are rolled back and the request is answered with a `GenericError`, so the controller is never left to infer that failure from arithmetic. A backup controller that needs given filesystems protected therefore requests exactly one mount point per required superblock (`guest-get-fsinfo` lists every mount point with its source device; entries sharing a device are one superblock) and accepts the result only when the count equals the number requested: any shortfall means a required target is missing, unsupported, unmatched or held by someone else, and the controller fails closed rather than guessing which. The agent does not narrow its own filesystem allowlist for this and discloses nothing beyond the count; the rest of the controller protocol (the validity of the interval the freeze protects) is §4.5.
