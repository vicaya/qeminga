...
1. *Discover.* `guest-get-fsinfo` lists every mount point with its source device; the controller picks one mount point per required superblock (entries sharing a `name` are one superblock), so an alias can never be requested twice.
