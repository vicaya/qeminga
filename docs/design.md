...
| AC20 | Freeze operation deadline (§4.4): with one target frozen and the next target's `FIFREEZE` blocked past `fsfreeze_operation_timeout_secs`, the frozen target is thawed through its descriptor while the call is still blocked, no further target is authorised, the request fails with `GenericError`, status keeps answering `frozen` and the marker stays until the blocked call returns; a late success is thawed through its own descriptor and never published as `Frozen`; a thaw, a stop or a terminal channel error during the operation joins or waits for it instead of draining or exiting on its own. |
