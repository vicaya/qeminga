...
stateDiagram-v2
    ...
    Freezing --> Thawed : nothing frozen and nothing held (empty or entirely skipped plan)
    Freezing --> Thawing : operation deadline expired or thaw requested; frozen targets recovered
...
Mount order is load-bearing: the freeze plan is traversed in reverse mount order so nested mounts are frozen deepest-first; thaw traverses it forward. Before the first `FIFREEZE`, qeminga switches audit output to the freeze-safe ring and creates the recovery marker described in §4.4. If marker creation fails, it performs no freeze ioctl. The walk is owned by a freeze *operation* (§4.4 "Operation deadline"), not by the request that started it: each target's `FIFREEZE` is a tracked blocking task of its own, a completed target's descriptor is published before the next target is authorised, and the operation has a deadline after which no further target is frozen and the completed ones are thawed while a blocked call is still awaited.
...
- `EBUSY` — the superblock is already frozen. This is non-fatal (for example, a concurrent freezer may hold it) and does **not** increment the freeze result: only successful `FIFREEZE` calls count as frozen by qeminga. It is retained in the thaw plan by deliberate compatibility policy (its descriptor is held and drained with the operation's own targets). Because nesting depth is unknowable during recovery, a recovery drain ("Thaw scope", below) drains all planned filesystems; a concurrent in-guest freezer is therefore unsupported and may have its freeze released by qeminga's later thaw.
...
**Zero-work operations:** an operation that ends with no target frozen, no descriptor held (no `EBUSY` target retained for the drain), nothing uncertain and no worker outstanding has nothing to recover: it removes the marker and settles `Thawed` with the reply `0`, never `Frozen` with a watchdog armed and the gate closed on nothing. This covers an empty plan (no eligible filesystem, or a `guest-fsfreeze-freeze-list` that matches nothing) and a plan every target of which was skipped. `0` alone is not the criterion: an `EBUSY` target, a lost worker, an incomplete drain or a marker that cannot be removed keeps the conservative state, and in the last case the reply is an error naming the retained marker rather than a `0` that reads as a clean settlement.

**Coverage:** the reply to `guest-fsfreeze-freeze-list` is the number of distinct superblocks, among those the requested mount points name, on which this operation's `FIFREEZE` succeeded, and nothing else (C-12, AC17). A requested mount point names exactly one thing: the superblock the pathname leads to *now*, resolved as the kernel resolves the path, from the root mount along the pathname's components, crossing into whatever `/proc/self/mountinfo` (mount ids and parents) shows attached at each point: a mount stacked on another, a mount moved over a newer one, a mount placed over a plain directory above another mount point are all followed, and the order of the table decides nothing. It never names a superblock whose mount point is hidden, by a mount over it or over any directory above it (that superblock is named by the mount points it is still visible at, if any), so one requested name selects at most one superblock. The table is indexed once per request and each distinct name resolved once, so a request costs the request plus the table, never their product (§5.10). The freeze then opens a selected superblock on the requested name and on nothing else, and the opened directory is verified to be on that superblock: a name that leads elsewhere fails the operation rather than being rescued by an alias, so the coverage a count certifies is the coverage of the names the controller gave. Aliases serve the recovery drain, which reaches and recovers a superblock through every mount point it has, hidden ones included ("Thaw scope", below). A requested path outside the plan, a target the kernel does not freeze (`EOPNOTSUPP`) and a superblock another freezer already holds (`EBUSY`, not this operation's freeze under the single-freezer assumption) are all left out of the count, and two names of one superblock (a bind alias) count it once. A planned target that cannot be reached at all (none of its mount points opens on its superblock) is not a lower count but a failed operation: the completed targets are rolled back and the request is answered with a `GenericError`, so the controller is never left to infer that failure from arithmetic. A backup controller that needs given filesystems protected therefore requests exactly one mount point per required superblock (`guest-get-fsinfo` lists every mount point with its source device; entries sharing a device are one superblock) and accepts the result only when the count equals the number requested: any shortfall means a required target is missing, unsupported, unmatched or held by someone else, and the controller fails closed rather than guessing which. The agent does not narrow its own filesystem allowlist for this and discloses nothing beyond the count; the rest of the controller protocol (the validity of the interval the freeze protects) is §4.5.
...
**Thaw scope:** which filesystems a thaw must drain depends on what is known about them. The thaw of a freeze operation this process completed, with no target's outcome unknown, drains exactly what the operation published: the descriptors of the targets it froze and of the `EBUSY` targets it retained, each opened and verified against its superblock at freeze time. No mount table is read for it and no other superblock is touched: one the operation never froze, whether hidden under a mount over its ancestor (`directory_overmount.txt`: A, reachable by no pathname while B covers `/data`, when the request froze C through `/data/nested`) or simply outside a `guest-fsfreeze-freeze-list` request, is not its obligation and cannot hold its thaw, so the marker goes as soon as the held descriptors are drained and the next cycle is admitted. Everything else is a *recovery drain* and discovers its targets through the mount table, every eligible superblock by its pathnames and aliases, as the paragraph above describes: a thaw after a restart (no descriptor survives), a thaw received while `Thawed`, and the thaw left by an operation whose outcome is uncertain (a freeze worker, a recovery drain, a rollback or a thaw's own drain task lost: a target may be frozen with no descriptor to show for it, so the descriptors that remain are no longer evidence of complete ownership). There the conservative rule stands: a planned superblock reached by no pathname is reported unreachable and the marker retained. A thaw that leaves a descriptor undrained keeps it, and its scope, for the next attempt (the watchdog's drain included); a completed thaw resets the scope to discovery, so nothing but a settled operation ever narrows a drain.
...
**Operation deadline (OQ-8, T6.1):** the watchdog bounds the time spent in `Frozen`, from the moment the freeze walk has completed, and it is re-armed on every entry into that state (after a failed thaw included). The walk itself is bounded separately, because `FIFREEZE` waits for the filesystem's writers and performs its own sync, the call cannot be interrupted, and abandoning its `spawn_blocking` handle would let a late completion freeze a target after the caller had given up. A freeze request therefore starts an *operation* that owns the walk independently of the request and of the channel. Preparation (mount table, marker) and every `FIFREEZE` run as tracked blocking tasks, one target at a time, and a completed target's verified descriptor is published before the next target is authorised. The operation's deadline is immutable, `fsfreeze_operation_timeout_secs` from the moment the request entered `Freezing` (omitted, the key derives `min(60, fsfreeze_max_timeout_secs)`, §8.2); queueing and preparation count against it and heartbeats do not extend it. The abort decision, a pending thaw request or the deadline read from one clock shared by the coordinator and its workers, is taken at every boundary: before each target is authorised, by the worker right before its ioctl, while a worker is awaited (the abort is polled ahead of a completion, so a chain of ready completions cannot postpone a pending abort), and before a successful completion is committed, so a completion arriving after the deadline enters recovery and is never published as `Frozen`. The worker's check is its last opportunity, not a boundary with the syscall: a worker past it is owned, and whatever it returns is recovered. When the abort commits (exactly once): no further target is authorised, the state moves `Freezing → Thawing` (the freeze token is consumed, so a late completion can never publish `Frozen`), the request receives its single reply (a `GenericError` naming the cause, the target in flight and the count frozen so far), and the completed targets are drained on a blocking task of their own, independent of the worker still blocked in `FIFREEZE`. Every authorised worker is awaited to its end: a late success or `EBUSY` is drained through the descriptor it opened, a late error needs no recovery, a lost worker leaves its target uncertain. The operation settles only when no worker is outstanding and every published descriptor has been drained: complete → marker removed, finalisation, `Thawed`; a drain incomplete or the marker not removable → `Frozen`, marker retained, the watchdog armed as on any entry into `Frozen`, the incomplete descriptors kept for it. The operation retires its registration by identity before the terminal state is published, so whoever observes that state finds the slot released, a successor can only be admitted afterwards, and the old operation's cleanup can never touch it; the audit finalisation always completes before `Thawed` admits a new freeze. Until then the marker, the frozen command gate (status answers `frozen`) and the freeze-safe audit mode remain, another freeze is refused, a thaw joins the operation rather than draining anything itself (it requests the abort and is answered at once, conservatively, with the recovery pending: it never waits on a drain, which could be blocked, and never reports a complete thaw while a call can still freeze a target; the host learns the outcome from `guest-fsfreeze-status`), and a stop or a terminal channel error waits for the settlement like any other exit (§5.7). Capacity is explicit: one operation at a time, and per operation at most one freeze worker, one recovery drain and the preparation task. The guarantee holds while the coordinator and the relevant kernel calls make progress: an overdue `FIFREEZE` no longer prevents the recovery of the targets completed before it. It is not a promise that a wall-clock deadline makes an unresponsive kernel or device writable: the deadline bounds the agent's reaction, not the completion of the kernel call, and a `FITHAW` that blocks still holds its drain (and the operation) until it returns; isolating several simultaneously stuck thaws from one another is the part of OQ-8 left open. For the same reason the operation deadline's relation to the hard cap (§8.4) bounds when the agent stops authorising work, not the total time a recovery can take.
...
### 4.5 Snapshot Controller Contract

A successful freeze reply is not a snapshot barrier of indefinite length, and `"frozen"` from `guest-fsfreeze-status` is not a readiness certificate. This section states what the replies mean and the one protocol a backup controller is supported to build on them (#43 §1); the reference controller in `tests/controller_contract.rs` is the executable form of it.

**What the agent promises.** The reply `N` to `guest-fsfreeze-freeze-list` says that, at the moment the reply was sent, this operation's `FIFREEZE` had succeeded on `N` distinct superblocks among the requested mount points (§4.2 "Coverage"), and that the watchdog lease of §4.4 was running: an idle timeout of `fsfreeze_idle_timeout_secs`, reset by every `guest-fsfreeze-status`, under a hard cap of `fsfreeze_max_timeout_secs` from the moment `Frozen` was published. When either expires the agent thaws on its own; a status heartbeat extends the idle timeout and nothing extends the cap. `guest-fsfreeze-status` answers `frozen` in `Freezing`, `Frozen` and `Thawing` alike (C-7): it is a liveness signal and the heartbeat, not a statement that the required filesystems are protected. The reply `M` to `guest-fsfreeze-thaw` counts the planned superblocks on which at least one `FITHAW` succeeded in that thaw, which under the single-freezer assumption of §4.2 is exactly the set still frozen when the thaw ran.

**The protocol.** One cycle protects one consistency point; a controller runs it as follows.

1. *Discover.* `guest-get-fsinfo` lists every mount point with its source device; the controller picks one mount point per required superblock (device-backed entries sharing a `name` are one superblock; pseudo filesystems share a name without being one, and cannot be frozen anyway), so an alias can never be requested twice. A mount point names the superblock mounted at it now and nothing else (§4.2 "Coverage"): a superblock hidden by a mount over its former mount point is named by the mount points it is still visible at, so one requested name can never count for two superblocks, and a name that leads nowhere counts for none. Under the data-protection profile (§5.9) `guest-get-fsinfo` is disabled and this step does not go through the guest at all: the required mount points are provisioned to the controller by the deployment (the same trusted configuration that chose the profile and the guest's filesystem layout), one per required superblock, and steps 2 to 5 are unchanged. Discovery is a convenience, never a source of trust: a provisioned or discovered mount point that is wrong is a shortfall in step 2 and the cycle is rejected.
2. *Freeze.* `guest-fsfreeze-freeze-list` with those mount points, under a client-side timeout of at least `fsfreeze_operation_timeout_secs` plus a margin. An error, a timeout, or a count different from the number requested is a rejection: the controller sends a thaw regardless, waits for `guest-fsfreeze-status` to answer `thawed`, and starts no new cycle before that.
3. *Cut.* The controller sends `guest-fsfreeze-status` at least every third of `fsfreeze_idle_timeout_secs`, measured from the instant the freeze request was *sent* (the agent's lease started no earlier than that, so this is conservative), requires every answer to be `frozen`, and establishes the consistency point of every required volume. The whole cycle, from the freeze request to the thaw reply, must fit inside `fsfreeze_max_timeout_secs` measured the same way; a controller whose cut cannot finish inside that budget stops cutting and proceeds to the thaw.
4. *Thaw.* `guest-fsfreeze-thaw`. An error is a rejection. A count different from the number requested is a rejection: fewer means a required superblock was no longer frozen when the thaw ran (the lease expired, a guest reboot, an external thaw), so the interval was not continuously protected; more means another freezer was at work and the assumption the count rests on does not hold.
5. *Verdict.* The consistency points established in step 3 are quiesced only when steps 2 and 4 both held and the thaw reply arrived inside the budget of step 3: a matching count says the kernel was still frozen when the thaw ran, not that it was this cycle's lease that held it (an agent restart re-arms the cap from its recovery, §4.4), so a reply past the budget is a rejection whatever the count. Copying or uploading an established snapshot happens after step 4 and is not bound by the lease; a verdict, once reached, is not revisited because a later upload ran long.

**Rules the protocol depends on.** Exactly one freeze per cycle: a controller never sends a second freeze to "extend" a lease, and a cycle that needs to be retried is a new cycle whose snapshots are taken again. A result is rejected whenever continuity cannot be established, not only when a failure is observed: a timed-out freeze, a lost connection or an agent restart before the freeze reply arrived, a thaw error, a thaw count that does not match. The thaw count is what ties the verdict to the original operation rather than to "some freeze being active": the watchdog's drain thaws every superblock the operation froze completely (`FITHAW` until the kernel answers "not frozen", §4.2), so once the lease has expired no requested superblock can be counted by the controller's later thaw; an agent restart while frozen changes nothing the kernel holds (the marker puts the new instance into recovery, and the thaw counts the same superblocks), whereas a guest reboot thaws everything and leaves the count at zero; and a second freeze cannot be admitted while an operation is unresolved (§4.4), so a freeze that is under way cannot be replaced under the controller's feet. What the count cannot see is a freeze issued by another party between the expiry and the controller's thaw; that is the single-controller assumption, stated as a limitation and the reason an opt-in operation identity is proposed rather than implied (OQ-9 in `docs/tasks.md`).

**What a cycle's thaw touches.** The thaw of a cycle drains what its freeze published and nothing else (§4.2 "Thaw scope"): a superblock the cycle never froze, whether hidden under a mount over its ancestor or outside the request, is not its obligation and cannot hold its thaw, so a cycle over a table that hides a filesystem still finalises (marker removed, `Thawed`) and the next cycle is admitted. The drain that discovers every planned superblock through the mount table is the recovery's: after a restart, for a thaw sent to a `Thawed` agent, and after an operation whose outcome is uncertain; there a planned superblock reached by no pathname keeps the marker, and a controller that meets it treats the guest as needing an operator.

**Timing assumptions, stated conservatively.** Heartbeat period at most a third of the idle timeout; cycle budget under the hard cap by a margin covering the freeze walk and the thaw drain (the operation deadline of §4.4 bounds the former; a `FITHAW` that blocks is OQ-8); both measured from the send of the freeze request on the controller's clock. A controller that misses a heartbeat may lose the lease and is told so by the thaw count; a controller that keeps heartbeating past the cap is thawed at the cap and told so by the thaw count. Neither can be mistaken for a quiesced interval.
...
| `CAP_SYS_BOOT` | `reboot(2)` syscall; kept only with `[features] shutdown = true` (§5.9) |
...
Capability sets and the bounding set are per thread, and the drop establishes them for the thread that performs it (the `setresuid` broadcast reaches every thread's UID, the filter of §5.5 is installed for all threads, but `PR_CAPBSET_DROP` acts on the caller only). The ceiling therefore holds process-wide by construction, not by broadcast: **every thread of the process is created after the drop**, by the dropped thread, and inherits its sets. Logging starts before the drop, but only its queue; the audit writer thread (§9.1) is started right after the drop (step 5b of the startup sequence, `Startup::start_audit_writer`), records written before then wait in the bounded delivery queue, and a refusal on the way out starts the writer to report itself. The runtime's threads start after the filter. The privileged tests read `/proc/<pid>/task/*/status` for every thread (the audit writer and the blocking-pool workers among them) under both profiles and require the same UID, capability sets, bounding set, `no_new_privs` and seccomp mode on each; the installed-artifact test does the same under the shipped unit.
...
| Privileged handlers | `ioctl` restricted by request number to `FIFREEZE` and `FITHAW`, plus `FITRIM` only with `[features] fstrim = true`; `sync` and `reboot` only with `[features] shutdown = true` (§5.9) |
...
Release builds use `panic = "abort"` and rely on the service manager to restart the process. A panic or seccomp kill never attempts a best-effort write or thaw from an indeterminate state; the pre-freeze marker forces the restarted process into the conservative recovery path in §4.4. A graceful stop requested while frozen is deferred until thaw only when the unit's `TimeoutStopSec` exceeds `fsfreeze_max_timeout_secs` plus a thaw-drain margin (§8.4); otherwise it intentionally degrades to the forced-stop recovery path. A forced stop leaves the marker in place. Once the stop is honoured, every destructive operation has completed (the stop waits for the thaw, for the commands running, and for an aborted freeze operation to settle, §4.4); the runtime is then shut down under a short bound rather than waited on indefinitely, so an abandoned informational walk (`guest-get-fsinfo` stuck in `statfs(2)` on a dead share, itself limited in number) can never hold the process open. The same rule governs every other way out of the serving loop: a terminal channel error (an `EBUSY` on a reopen, §8.4) ends the serving, since the port is not competed for, but the process exits only once the state is `Thawed`, so a freeze it holds is released by the watchdog (§4.4, the only recovery left once no host can reach the process) and a thaw in flight completes before the runtime is torn down; the terminal error is still reported.

A session keeps three concerns apart: admission, completion and delivery. *Admission* is by lane: every command that may change the guest (a freeze, a trim, a shutdown, a suspend, a walk, an unknown method) runs in one serial lane, one at a time and in request order among themselves, so their side effects happen in the order the host asked for them and a freeze never starts while an earlier operation is still active; the frozen-safe controls (`guest-fsfreeze-status`, `guest-ping`, `guest-sync`, `guest-sync-delimited`, `guest-info`, and a frame that is answered without running anything) run beside the command in progress, at most three at once; and a `guest-fsfreeze-thaw` runs beside a freeze in progress, to reach its operation before the deadline (§4.4), and in the serial lane otherwise. A control, or a thaw while nothing but a freeze runs, is not held back by a serial command waiting ahead of it for the lane: a walk the freeze gate would refuse anyway, queued between a freeze and the thaw meant to abort it, does not delay the thaw. Every command still passes the allowlist, the feature, rate and freeze gates and produces its one audit record; the lane is decided by a side-effect-free parse before the command starts. The session's buffering is bounded end to end. The queue has eight places, each owned by one command from its frame's decoding to its reply's delivery, in request order: a reply keeps its place until the host has taken it, not merely until its handler finished; a frame decoded beyond the places (one read's worth at most) waits for one; and a command that overtakes an earlier one can never use up the place the earlier one needs to start once its lane frees, so later replies cannot wedge the queue behind a waiting command. On top of the places, a reply budget of 64 KiB is a backpressure threshold, not a ceiling: once the undelivered replies reach it nothing more is started, and nothing more is read once the finished replies reach it either, while the replies of the commands already started, and a single reply larger than the budget, are still kept whole. A peer that stops reading therefore stops being read, and the memory it can pin is the budget plus the replies of the commands that had a place when it was reached; the bounds hold back new work only, never the completion of what runs. The price is explicit: a thaw sent behind seven unanswered commands is read only once the freeze reply is out, at the latest at the operation deadline. *Completion*: every command that started is polled until it finishes, whatever the writer or a stop are doing, so a handler's continuation after its kernel work (publishing `Thawed`, finalising the audit) never waits for the host to read an earlier reply. *Delivery*: one writer emits the replies strictly in request order whatever order the commands finish in, so requests without an `id` keep their correlation and a `guest-sync-delimited` reply follows every earlier reply, which the host discards up to the sentinel (a resynchronisation therefore waits for a pending freeze reply, at most until the operation deadline); a write the host is slow to accept is resumed, never restarted, and stalls nothing but the replies behind it. A stop the state allows (`Thawed`) drains the session: nothing more is read or started, the commands running are finished, never dropped (a blocking kernel call cannot be cancelled by dropping its handler), and the session then ends, abandoning a reply the host is not accepting; if a command that was running froze the guest meanwhile, the stop is withdrawn until the thaw (C-21). Channel EOF or HUP lets the commands received finish (their side effects complete before the session ends; the replies are best-effort), then closes both channel handles, discards any partial frame, and retries opening the configured channel with bounded backoff; the same loop performs the first open when the pre-drop attempt of §5.4 found no device. Reconnection never resets `FreezeState`, cancels a watchdog, clears the recovery marker, or enables the normal audit sink while frozen. Recovery-mode startup (§4.4) does not wait for the channel: the watchdog is armed as soon as the runtime starts, so an abandoned freeze is bounded even if the host never connects. A newly connected peer starts with a clean decoder and should use `guest-sync-delimited` to establish stream synchronisation.
...
### 5.9 Authority Follows the Configuration; Residual Authority

**The data-protection profile (#43 §5).** Every command family that is not needed for snapshot coordination is a runtime switch in `[features]`: `fstrim` (D2), `suspend_ram` (D1), and since this section `shutdown` (`guest-shutdown`) and `information` (`guest-get-osinfo`, `guest-network-get-interfaces`, `guest-get-fsinfo`). The defaults keep the lifecycle-oriented behaviour of an existing installation; `packaging/config-data-protection.toml` turns the four off and keeps freeze, thaw, status, ping, sync and `guest-info`, which are never optional because the agent must always own its recovery. A disabled command answers `CommandNotFound` with the upstream `has been disabled` description (C-2), before the freeze gate and without a handler running, and is listed with `"enabled": false` by `guest-info` (§3.1). The static command ceiling of §5.1 is unchanged: a profile can only narrow it.

**The controller under this profile.** §4.5's protocol starts by discovering mount points with `guest-get-fsinfo`, which this profile disables. The supported workflow is provisioning: the deployment that installs the profile also tells its backup controller the required mount points (a fixed guest image has a fixed layout; a provisioned guest knows the mount points it created), one per required superblock, and the controller runs steps 2 to 5 of §4.5 on them. Nothing in those steps needs the information commands: the freeze count says whether every provisioned mount point named a superblock that was frozen, the thaw count whether each was still frozen, and a provisioned mount point that is wrong (moved, hidden by a mount over it, unsupported) is a shortfall and a rejection, exactly as a discovered one would be. Discovery is not restored for this; the profile's point is that the host learns nothing about the guest but the count. `a_cycle_under_the_data_protection_profile_needs_no_discovery` (`tests/controller_contract.rs`) runs the reference controller this way.

**Authority is derived, not merely hidden.** Before the first host request is accepted, the configuration yields the process's *authority* (`Config::authority`): with `shutdown` off the capability drop of §5.4 keeps only `CAP_SYS_ADMIN` and `CAP_DAC_READ_SEARCH` (`CAP_SYS_BOOT` is trimmed from the bounding set like every other non-final capability, so it cannot be regained) and the seccomp profile of §5.5 omits `reboot` and `sync`; with `information` off the profile omits `uname`, `statfs`/`fstatfs`, the netlink syscalls and `socket` altogether; with `fstrim` off `FITRIM` is not an allowed `ioctl`. The filter is installed before the runtime starts and the configuration is read from the guest, so no host traffic can widen the profile, and a profile change between two starts strands nothing: the recovery marker, the thaw drain and the watchdog use only what every profile keeps.
...
└── src/
    ...
    ├── freeze_op.rs       # freeze operation coordinator: deadline, abort, late completions
...
shutdown    = true    # opt-out: guest-shutdown; off, CAP_SYS_BOOT and reboot(2) are given up too (§5.9)
information = true    # opt-out: guest-get-osinfo, guest-network-get-interfaces, guest-get-fsinfo (§5.9)
```
...
The shipped systemd unit deliberately does not bind to the virtio-port device unit: a service held by `BindsTo=`/`After=` on a missing device would never reach the recovery of §4.4 after a crash while frozen, so the daemon makes one privileged open attempt and its reopen loop (§5.7) retries once the device appears. The unit starts before ordinary multi-user workloads. `/run/qeminga` is created at boot by a `tmpfiles.d` rule and re-provisioned by privileged `ExecStartPre` lines before every start, owned by the `qeminga` account (not through `RuntimeDirectory=`, which with no `User=` re-applies root ownership before every `ExecStart` and would leave the dropped daemon unable to create or remove its marker). This lets the recovery marker use `openat(..., O_CREAT|O_EXCL)` without a runtime `mkdirat`. For the default `fsfreeze_max_timeout_secs = 300`, it sets `TimeoutStopSec=330s`; any change to the hard cap must keep `TimeoutStopSec` greater than that cap plus a thaw-drain margin, or service-stop deferral deliberately falls back to marker recovery (§5.7). `fsfreeze_operation_timeout_secs` (§4.4) is validated to be at most the hard cap, so the same figure bounds the deferral of a stop received during a freeze walk.

At startup, an `EBUSY` opening the channel is a terminal, explicit `channel_already_open` error, not an in-process retry. This makes a boot-time single-open race with a leftover agent, debugger, or local process visible to the operator instead of presenting as a healthy but unreachable agent. An `EBUSY` on a later reopen is terminal for the channel in the same way, but while a filesystem may be frozen the process stays until it is thawed (§5.7).
...
**No sink I/O on the agent's own paths (#43 §3).** The sink is written by one dedicated writer thread and by nothing else; the thread is started after the privilege drop (§5.4), and until then records wait in the queue. Outside a freeze window a record is placed in a byte-bounded delivery queue (256 KiB) and the caller returns; entering the ring and flushing it back only move records between the ring and the queue (the ring's loss record first, then its records in order, behind whatever was queued before the window), so the thaw's finalisation, the coordinator's settlement and the watchdog's drain never wait for journald, and the terminal state and the marker are finalised whether or not a single record has been delivered. While the mode is the ring the writer thread parks even with records queued from before the window, so nothing is written between the first `FIFREEZE` and the thaw; a write already in flight when the window opened completes on that thread alone. A sink that stops reading therefore blocks the writer thread and nothing else; records that arrive while the queue is full are dropped rather than the daemon held, and records the sink refuses are dropped too. Every loss is accounted for where it happened: a loss record (`audit_records_lost`, with `lost` and a `reason` of `ring_overflow`, `sink_backpressure` or `sink_error`) is delivered at the position of the gap, and a loss record the sink refuses is kept until the sink accepts something again, without spinning on a dead sink. A gap holds a count per reason and adjacent losses merge into one gap whatever their reasons, so the queue never holds more than one gap between two records and its entries, not only its bytes, are bounded by what the byte budget admits: a freeze window overflowing behind a full queue adds to the gap already there rather than appending two markers per window. The writer owns the item it is delivering: it is taken out of the queue for the write and stays counted (its bytes, its losses) until delivered or put back, so a loss recorded while a gap's records are on their way to the sink starts a new gap behind it and is never written off with the old one; reported plus unreported losses always equal the losses that happened. Delivery is in order, the queue being FIFO. `guest-shutdown` gives its own audit record a bounded chance (2 s) to be delivered before `reboot(2)`; a blocked sink delays the shutdown by that much and no more, and the shutdown path touches no descriptor itself (the writer thread may be blocked in a write to stderr, holding the process's stderr lock, so nothing on that path flushes stderr). The daemon's exit paths, a refusal after logging started and the end of serving alike, flush the ring and give delivery the same grace: a refusal that came after recovery logging started (§4.4) is reported on the way out rather than left in a ring nothing will drain, and the global subscriber's handle, which lives as long as the process, is never relied on for a last-handle drain.
...
| D8 | `guest-shutdown` and the information commands are runtime switches (`[features] shutdown`, `information`, both on by default) so a data-protection deployment can turn them off; the capability set and the seccomp profile follow the switches (§5.9). |
...
| AC16 | Against real libvirt, `virsh domfsfreeze`, `virsh domfsthaw`, `virsh domifaddr --source agent`, and `virsh shutdown --mode agent` succeed. |
...
| AC20 | Freeze operation deadline (§4.4): with one target frozen and the next target's `FIFREEZE` blocked past `fsfreeze_operation_timeout_secs`, the frozen target is thawed through its descriptor while the call is still blocked, no further target is authorised, the request fails with `GenericError`, status keeps answering `frozen` and the marker stays until the blocked call returns; a late success is thawed through its own descriptor and never published as `Frozen`; a thaw, a stop or a terminal channel error during the operation joins or waits for it instead of draining or exiting on its own; and through the production channel loop a thaw sent while the freeze reply is pending reaches the operation, status is served while a recovery drain is blocked, and replies keep request order. Evidence is a gated kernel double and a manual clock; no real blocked `FIFREEZE` is reproduced. |
