...
| OS and network information (only with `[features] information = true`, §5.9) | `uname` for `guest-get-osinfo`; `statfs`/`fstatfs` for `guest-get-fsinfo`; `socket` (AF_NETLINK only), `bind`, `sendto`/`sendmsg`, `recvmsg`, and `getsockname` for the netlink exchange used by `guest-network-get-interfaces` |
