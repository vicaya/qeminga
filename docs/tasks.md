...
- **Tests first (red):**
  ...
  - `privileged_data_protection_profile_holds_no_reboot_authority` (`tests/privileged_e2e.rs`: the real daemon under the profile shows `CapEff`/`CapPrm`/`CapBnd` `0000000000200004`, `Seccomp: 2`, answers "disabled" to the six commands, is still alive after a disabled shutdown, freezes and thaws the loop ext4).
