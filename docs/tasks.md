...
- **Depends on:** T6.2
- **Files:** `docs/design.md` §4.5, `tests/controller_contract.rs` (new), `src/kernel/fake.rs` (`track_freeze_depth`, `reset_freeze_depths`: the fake models the kernel's nesting, so a thaw count reflects what was frozen).
- **Tests first (red)** (`tests/controller_contract.rs`: the reference controller `Cycle` driven against the production dispatcher, coordinator and watchdog on paused time, one agent instance per "process" over a shared kernel and marker directory):
  ...
  - `an_agent_restart_while_frozen_keeps_the_verdict_sound` (recovery mode over the new connection; the kernel kept the freezes), `a_guest_reboot_during_the_cut_is_rejected`, `a_crash_before_the_freeze_reply_is_rejected` (no count ever learned; the recovery drain over the restarted agent).
...
| AC22 freeze-validity contract | T6.3 (`tests/controller_contract.rs`, the thirteen scenarios listed there) | integration (paused time, depth-tracking fake) | green; single-controller assumption stated, OQ-9 |
