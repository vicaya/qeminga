...
- **Tests first (red)** (`tests/controller_contract.rs`: the reference controller `Cycle` driven against the production dispatcher, coordinator and watchdog on paused time, one agent instance per "process" over a shared kernel and marker directory):
  ...
  - `a_new_cycle_after_an_expired_one_quiesces_only_its_own_snapshots`, `a_foreign_refreeze_after_the_expiry_is_the_single_controller_assumptions_limit` (the boundary of the count, pinned; the agent's refusal of a second freeze while one is unresolved), `an_external_thaw_behind_a_frozen_status_is_rejected` (`frozen` is not a certificate), `a_request_that_is_not_fully_covered_is_rejected_before_any_cut`, `a_hidden_mount_point_cannot_stand_in_for_an_unprotected_requirement` (the external review's counterexample on `hidden_mount.txt`: `/data` over an overmount plus an unfreezable `/required` is a count of 1, rejected; with the old selection the hidden name made it 2 and the cycle was accepted), `a_mount_moved_over_a_newer_one_is_what_a_request_protects` (`moved_mount.txt`: A moved over B keeps its earlier row; the cycle protects A, opened on `/data`, B untouched, quiesced) and `a_requirement_hidden_by_a_mount_over_its_ancestor_is_rejected` (`hidden_nested.txt`: 0 of 1).
