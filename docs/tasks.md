...
| OQ-7 | C-14 says recovery mode arms the watchdog "immediately", but `main` opens the channel before the runtime exists, and a non-`EBUSY` open failure (device missing) is retried with a blocking sleep, so `start_recovery` runs only once the channel is open. Under systemd `BindsTo=` on the port device makes the window narrow; is a bounded number of attempts before arming acceptable, or should the runtime start first? | T4.6 | Keep the §5.4 order (channel before drop, drop before seccomp, runtime last); document the window here. |
