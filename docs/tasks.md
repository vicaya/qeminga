...
| C-23 | The freeze operation deadline (§4.4, #39) is `[agent] fsfreeze_operation_timeout_secs`, default **60** (libvirt gives up on a freeze after a few seconds, while a legitimate walk that must sync a large dirty page cache can take tens of seconds; a walk still inside `FIFREEZE` after a minute is treated as stuck), validated to `1..=86400` and at most `fsfreeze_max_timeout_secs`, so `TimeoutStopSec` stays coupled to one figure (§8.4). It is measured from entry into `Freezing`; queueing and preparation count, heartbeats do not extend it. A `guest-fsfreeze-thaw` received while the walk is under way aborts the walk the same way (the host has given up) and joins the recovery instead of being refused with "cannot thaw while freezing". The reply to an aborted freeze is a `GenericError` naming the cause, the target in flight and the count frozen so far; the operation keeps the marker, the gate and the ring until it settles. Chosen when implementing #39; the design owner may retune the default. |
...
- **Tests first (red):**
  ...
  - `the_ring_stays_until_an_aborted_freeze_has_settled` (`tests/audit_freeze_window.rs`), `a_stop_during_an_unresolved_freeze_waits_for_its_settlement` (`tests/startup.rs`), `abort_freeze_moves_freezing_to_thawing_with_a_thaw_token` (`state`), `operation_timeout_is_bounded_by_the_hard_cap_and_positive` (`config`), `a_gated_freeze_waits_outside_the_fake_mutex_until_released` and `a_gated_thaw_waits_and_a_scripted_error_still_applies_after_release` (`kernel::fake`).
- **Implement (green):** as §4.4 "Operation deadline": a driver task per operation (`tokio::spawn`, never aborted) that spawns one tracked blocking task per target, publishes each completed handle to `Context::frozen_mounts` before authorising the next, watches the deadline and the abort request with `tokio::select!` while awaiting a worker, commits the abort once (`abort_freeze`, one reply through a `oneshot`, `on_thaw_claimed`), drains published handles on a separate blocking task (`drain_held`), awaits every worker to its end, and settles through the thaw token (`Thawed` after `on_thawed` and marker removal, or `Frozen` with `on_frozen`). Progress is published through a `watch` channel that the joining thaw waits on. Mutation gate: `src/freeze_op.rs` is in `.cargo/mutants.toml`.
...
| AC20 freeze operation deadline | T6.1 (`a_completed_target_is_thawed_while_a_later_fifreeze_is_still_blocked` and the scenario tests listed there; `tests/audit_freeze_window.rs`, `tests/startup.rs`) | unit (real time, gated fake) | green; no privileged reproduction of a blocked `FIFREEZE` |
