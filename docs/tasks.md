...
| C-18 | If the process is not started as root, `main` skips the capability drop and logs a warning that freeze/trim/shutdown will fail with `EPERM`. This is what makes the unprivileged end-to-end tests (T4.7) possible without changing production behaviour. Since T6.5 the skip is allowed only under `hardening = "unenforced-development-only"`: under the default, enforced profile a drop that did not run is a startup refusal (§8.1). |
...
| C-25 | `[agent] hardening` (§8.1, #43 §4) defaults to `"enforced"`: the daemon refuses to serve (exit 78, one line, the marker untouched) unless started as root with the seccomp filter compiled in, enabled and installed enforcing, and refuses any `QEMINGA_TEST_FAKE_KERNEL`. The opt-out is the one other value, `"unenforced-development-only"`; anything else is a parse error. Existing deployments built without the `seccomp` feature therefore stop starting after this change until rebuilt with it (or, on a development host, opted out): that is the point, and the packaging README says so. The e2e harness writes the opt-out by default; the enforced privileged run uses the default. `test-fakes` is a compile error outside a debug build. |
| C-26 | `[features] shutdown` and `[features] information` (both default `true`) are the runtime switches for `guest-shutdown` and for `guest-get-osinfo`/`guest-network-get-interfaces`/`guest-get-fsinfo` (D8, §5.9, #43 §5); disabled commands answer `CommandNotFound` "has been disabled" like `fstrim` (C-2). `Config::authority()` is what the capability drop and the seccomp profile take: `CAP_SYS_BOOT`, `reboot` and `sync` only with `shutdown`; `uname`, `statfs`/`fstatfs`, the netlink syscalls and `socket` only with `information`; `FITRIM` only with `fstrim`. `guest-suspend-ram` has no syscall or capability of its own. `packaging/config-data-protection.toml` is the profile with all four off. |
...
| OQ-8 | The watchdog bounds the `Frozen` state only (§4.4 "Operation deadline"): a freeze walk blocked inside `FIFREEZE` on one target left the targets frozen earlier in the walk without an autonomous release until the call returned. **Partly resolved (T6.1, #39):** the walk is owned by an operation with its own deadline; completed targets are published incrementally and drained on independent capacity when the deadline expires or a thaw is requested, a late completion is drained through its descriptor and never published as `Frozen`, and the operation settles only when all work and recovery are done. **Still open:** a `FITHAW` that blocks holds its drain and the operation until it returns; several simultaneously stuck thaws are not isolated from one another, and no wall-clock deadline can make an unresponsive kernel or device writable. A separate policy (per-target drain workers with an explicit bound) would be needed for that. | T3.4, T3.5, T6.1, §4.4 | Implemented for the freeze walk as in §4.4; the residual thaw-walk limitation is stated there and left to the design owner. |
| OQ-9 | The snapshot controller contract (§4.5, #43 §1) rests on the thaw count and therefore on a single controller: a freeze issued by another party between the expiry of the controller's lease and its thaw is counted as if it were the controller's own (`a_foreign_refreeze_after_the_expiry_is_the_single_controller_assumptions_limit`). Existing QGA replies cannot distinguish the two. **Proposal (opt-in, bounded, no response shape changed):** an optional `operation` argument, a controller-chosen unsigned 64-bit nonce, on `guest-fsfreeze-freeze`/`-freeze-list` and on `guest-fsfreeze-thaw`. A freeze records it in the operation and in the recovery marker's content (8 bytes, so it survives an agent restart and is unambiguous across reconnects); a thaw carrying a nonce is refused with `GenericError` `operation mismatch` unless the freeze currently held (or recovered from the marker) carries the same one, and the controller rejects on that error; a thaw without the argument keeps today's semantics, and a freeze without one is held under no nonce, so a thaw with one is refused. libvirt never sends the argument and is unaffected. Needs the design owner's approval of the argument (§3) and of the marker carrying content (§4.4). | T6.3, §4.5, §3 | The count-based protocol of §4.5 with the single-controller assumption stated; nothing is added to the agent. |
| OQ-10 | After the drop the process still holds `CAP_SYS_ADMIN`, reads any file its uid and `CAP_DAC_READ_SEARCH` reach with the `openat`/`read` every profile keeps, and writes to the host channel (§5.9 "Residual authority"). Syscall filtering is not a file-content confidentiality boundary after a parser-process compromise. Is that a required guarantee? If so, a separately scoped design is needed: tighter file/descriptor confinement (a pre-opened set of directories and `openat2` with `RESOLVE_BENEATH`, or Landlock) and/or a small privileged broker holding the capabilities, to which the parser process hands *operations on planned targets* over a socketpair and which authorises each against its own plan, never a path or a syscall chosen by the parser. Not a prerequisite for #43 §1–§4 and no evidence of an exploitable parser defect is claimed. | §5.4, §5.5, §5.9 | Documented as residual authority; no confinement beyond the drop and the filter. |
...
- **Status:** done (the bound covers the `Frozen` state; the freeze walk itself is bounded by the operation deadline of T6.1, and a blocked `FITHAW` remains OQ-8)
...
### Phase 6 — Post-release follow-ups

#### T6.1 — Freeze operation coordinator with an operation deadline (#39)
- **Status:** done
- **Design:** §4.2 (`Freezing → Thawing`), §4.4 "Operation deadline", §5.7 (stop waits for settlement), §8.2 (`fsfreeze_operation_timeout_secs`), §8.4, AC20, C-23, OQ-8 (freeze-walk part).
- **Depends on:** T3.4, T3.5, T4.6
- **Files:** `src/freeze_op.rs` (new), `src/handlers/fsfreeze.rs` (`run_freeze` starts the operation; `thaw` joins one in progress; `drain_held` factored out of the rollback), `src/state.rs` (`abort_freeze`), `src/dispatch.rs` (`Context::freeze_op`, `with_freeze_operation_timeout`), `src/config.rs`, `src/kernel/fake.rs` (`Gate`, `script_freeze_gate`, `script_thaw_gate`: per-target barriers outside the fake's mutex, since the hook runs under it), `packaging/config.toml`, `tests/fixtures/config/`.
- **Tests first (red)** (unit tests on the current-thread runtime with a `ManualClock` and the fake's gates: expiry happens where the test puts it; the multi-thread transport tests order their events with gates too):
  - `a_completed_target_is_thawed_while_a_later_fifreeze_is_still_blocked` — A frozen, B blocked at a gate, the clock moved past the deadline: the request fails with the cause and the in-flight target, A is drained through its handle while B is still blocked, C is never opened, status answers `frozen`, a freeze is refused, marker and ring stay; B's late success is drained through its own handle, then marker gone and `Thawed`, `frozen` hook never fired, a new freeze accepted.
  - `a_late_error_after_the_abort_needs_no_recovery_and_ebusy_is_drained`, `expiry_during_preparation_freezes_nothing_and_leaves_no_marker` (a gated mount-table read; no ioctl ever, the late marker removed), `a_blocked_recovery_drain_keeps_status_served_and_spawns_nothing_more` (A's `FITHAW` gated: status served, a thaw joins the same pass, a freeze refused, one worker and one drain), `a_thaw_during_the_walk_aborts_it_and_joins_the_recovery` (a second thaw drains nothing again), `an_incomplete_recovery_drain_settles_frozen_with_the_marker_and_the_watchdog` (A's handle kept for the next drain), `repeated_commands_during_an_unresolved_operation_add_no_work`; the abort/commit handoff (review follow-up): `abort_and_commit_compete_on_one_transition` (`freeze_op`), `a_thaw_that_finds_a_committed_operation_takes_the_ordinary_path` (the thaw request loses the race with the final commit: told `Committed`, it thaws the frozen guest normally, never "recovery pending"), `a_thaw_accepted_before_the_commit_makes_the_driver_recover`; the boundaries without an await to notice them (mutation survivors of the CI gate): `the_deadline_passing_between_a_completion_and_the_next_authorisation_opens_nothing` and `the_deadline_passing_between_the_last_completion_and_the_commit_enters_recovery` (the driver held on the held-handles lock between a completion and its next decision while the clock passes the deadline, on the multi-thread runtime), `the_deadline_instant_itself_authorises_nothing` (`freeze_op`: the worker's last check and the driver's decision agree that the deadline instant is expired).
  - Decision boundaries (review follow-up): `a_completion_and_the_expiry_ready_together_abort_the_walk`, `a_worker_released_after_the_expiry_issues_no_ioctl`, `an_already_expired_deadline_authorises_no_target_at_all`, `the_last_completion_after_the_expiry_enters_recovery_not_frozen`; retirement: `retiring_an_operation_never_clears_its_successor` (`dispatch`), `rapid_freeze_thaw_refreeze_keeps_one_operation_registered`; the clock: `the_manual_clock_wakes_sleepers_only_when_advanced` (`freeze_op`).
  - Transport (`tests/channel_freeze.rs`, the production dispatcher and session loop over a duplex stream with the gated fake): `a_thaw_sent_while_the_freeze_reply_is_pending_aborts_the_walk`, `status_and_ping_are_served_while_a_recovery_drain_is_blocked`, `replies_keep_request_order_for_id_less_and_delimited_requests`, `repeated_controls_while_a_reply_is_pending_are_bounded_and_answered_in_order`, `a_lost_connection_leaves_the_operation_owned`; the loop itself (`channel`): `later_commands_run_while_a_reply_is_pending_and_replies_keep_request_order`, `at_most_max_in_flight_commands_run_at_once_and_the_rest_wait_their_turn`, `eof_finishes_the_commands_in_flight_before_the_session_ends`., `a_thaw_behind_answered_controls_still_aborts_the_pending_freeze` (the fifth command), `a_freeze_is_not_started_while_a_trim_is_running` (the gated `FITRIM`: no target opened, no operation registered, every `FITRIM` before the first `FIFREEZE`), `a_blocked_reply_does_not_stall_the_thaw_behind_it` (a 16-byte stream the host stops reading: the thaw behind the stuck `guest-info` reply drains and publishes `Thawed`, then a stop ends the session), `a_stop_during_a_blocked_reply_waits_for_the_running_trim`, `a_thaw_behind_a_waiting_walk_still_aborts_the_pending_freeze` (a walk waiting for the serial lane between the freeze and a thaw sent in a later read does not hold the thaw back; it is refused while thawing, never started), `a_thaw_finishes_while_the_outbox_is_saturated` (MAX_QUEUED held with the host reading nothing: a thaw inside `FITHAW` behind a stuck reply still publishes `Thawed`; every reply then arrives once, in order); session lanes (`channel`, a counting fake with prefixed lanes): `a_control_runs_while_a_freeze_reply_is_pending_and_replies_keep_request_order`, `an_ordinary_command_waits_for_the_one_running_before_it`, `a_thaw_runs_beside_a_freeze_but_not_beside_an_ordinary_command`, `at_most_max_controls_run_beside_the_command_in_progress`, `a_thaw_behind_finished_controls_still_reaches_the_pending_freeze`, `the_queue_is_bounded_and_a_frame_behind_a_saturated_backlog_waits_for_the_reply`, `a_blocked_reply_does_not_stall_the_commands_behind_it`, `a_stop_during_a_blocked_reply_finishes_the_running_command_and_starts_nothing`, `a_stop_accepted_while_a_freeze_runs_is_withdrawn_once_it_froze`, `eof_finishes_the_commands_received_before_the_session_ends`, `a_thaw_behind_a_waiting_serial_command_still_reaches_the_freeze`, `undelivered_replies_hold_their_place_until_the_host_takes_them` (a reply counts against `MAX_QUEUED` until the host has taken it), `the_reply_budget_pauses_the_session_before_the_count_does` (`MAX_REPLY_BYTES`), `bypassed_replies_cannot_consume_the_head_waiters_admission_credit` (ten frames in one read: a freeze, a waiting ordinary command, a thaw overtaking it and pending after the freeze reply is taken, seven controls; the ordinary command keeps its place and runs once the thaw is done, replies in order), `controls_overtaking_a_waiting_trim_leave_it_its_place` (`tests/channel_freeze.rs`, the same through the production dispatcher with a thaw inside `FITHAW` and a trim waiting for the lane); `lanes_follow_the_frozen_safe_set`, `classification_parses_without_side_effects` (`dispatch`)
  - Configuration: `operation_timeout_is_bounded_by_the_hard_cap_and_positive`, `an_omitted_operation_timeout_is_derived_under_the_hard_cap` (`config`), `a_configuration_without_the_operation_timeout_and_a_short_cap_starts` (`tests/startup.rs`).
  - `the_ring_stays_until_an_aborted_freeze_has_settled` (`tests/audit_freeze_window.rs`), `a_stop_during_an_unresolved_freeze_waits_for_its_settlement` (`tests/startup.rs`), `abort_freeze_moves_freezing_to_thawing_with_a_thaw_token` (`state`), `a_gated_freeze_waits_outside_the_fake_mutex_until_released` and `a_gated_thaw_waits_and_a_scripted_error_still_applies_after_release` (`kernel::fake`).
- **Implement (green):** as §4.4 "Operation deadline": a driver task per operation (`tokio::spawn`, never aborted) that spawns one tracked blocking task per target, publishes each completed handle to `Context::frozen_mounts` before authorising the next, takes the abort decision (`FreezeOp::abort_due`: a pending request, else the deadline on the injected `FreezeClock`) before each authorisation, in the worker before its ioctl, while awaiting a worker (abort branch first in `tokio::select!`) and before committing success, commits the abort once (`abort_freeze`, one reply through a `oneshot`, `on_thaw_claimed`), drains published handles on a separate blocking task (`drain_held`), awaits every worker to its end, and settles through one path (`conclude`: finalisation, the terminal state through whichever token it holds, the reply, then `Context::retire_freeze_op` by identity). Progress is published through a `watch` channel. The abort request and the driver's commit compete on one compare-and-swap (`FreezeOp::request_abort` → `AbortRequest::{Accepted, Committed}`); a thaw told `Committed` waits for the settlement and takes the ordinary thaw path. The session loop (`channel::run_session_until`, §5.7) admits frames by lane (`Handle::classify` → `channel::Kind`: one serial lane for everything that may change the guest, in request order among themselves; `MAX_CONTROLS` frozen-safe controls beside it and a thaw beside a freeze, neither held back by a serial command waiting ahead of it), has `MAX_QUEUED` places each owned by one command from decoding to delivery in request order (an undelivered reply keeps its place; a frame beyond the places waits for one; an overtaking command never takes the place of the one it overtook), and a `MAX_REPLY_BYTES` backpressure threshold on undelivered replies past which nothing more is started or read, polls every started command to its end with one `Outbox` (a queue of undelivered replies, released as the host takes them) in the same `select!` as the writer and the stop, and drains on a stop (nothing read or started, the running commands finished, a reply the host is not accepting abandoned). Mutation gate: `src/freeze_op.rs` is in `.cargo/mutants.toml`.
...
- **Status:** done
- **Design:** §4.2 "Zero-work operations", "Coverage" and "Thaw scope", AC17, AC21, C-12, C-24.
- **Depends on:** T6.1
- **Files:** `src/freeze_op.rs` (`settle_nothing_frozen`, `Driver::uncertain`), `src/freeze_plan.rs` (`MountTree`), `src/dispatch.rs` (`ThawScope`), `src/handlers/fsfreeze.rs` (`FreezeFailure::NothingFrozenMarkerRetained`, `run_thaw`, `thaw_blocking`), `tests/fixtures/mountinfo/no_freezable.txt`, `tests/e2e/mod.rs` (`SpawnOptions::recovery_marker`), `tests/e2e_lifecycle.rs`.
- **Tests first (red):**
  ...
  - `a_requested_name_selects_at_most_one_superblock_so_the_count_is_coverage` (`freeze_plan`) and `freeze_list_over_an_overmount_counts_the_visible_superblock_only` (`handlers::fsfreeze`), on `hidden_mount.txt` — the external review's counterexample: 8:3 mounted over 8:2 at `/data`; a request for `/data` and an unsupported `/required` must count 1, not 2. A requested name selects the superblock its pathname leads to now; a hidden former name selects nothing; the selected target keeps its aliases for the thaw's reach and recovery.
  - `a_mount_moved_over_a_newer_one_is_what_the_pathname_leads_to`, `a_mount_under_an_overmounted_ancestor_is_hidden`, `a_mount_over_a_directory_hides_the_mounts_beneath_it` (`freeze_plan`, on `moved_mount.txt`, `hidden_nested.txt` and `directory_overmount.txt`: the last is a mount over a plain directory above another mount point, which parent links alone miss) and `resolution_costs_the_request_plus_the_table_not_their_product` (8 192 copies of a missing name against 20 000 rows), `freeze_list_freezes_the_mount_over_a_directory_not_the_one_it_hides`, `freeze_list_freezes_what_the_requested_pathname_leads_to_not_a_later_row` and `a_requested_pathname_that_does_not_lead_to_the_selected_superblock_fails_the_operation` (`handlers::fsfreeze`), `privileged_a_mount_moved_over_a_newer_one_is_what_the_request_freezes` (`tests/privileged_e2e.rs`: the arrangement on real mounts in a private tmpfs, two loop filesystems, A moved over B; A is frozen, B is not, proven by `FIFREEZE` on the superblocks themselves) — the external review's second counterexample: the last row at a path is not the visible mount.
  - `a_self_parented_root_is_the_root_of_the_namespace` (`freeze_plan`) and `freeze_list_under_a_self_parented_root_freezes_the_requested_target` (`handlers::fsfreeze`), on `self_parented_root.txt` — the external review's fourth-round counterexample: proc_pid_mountinfo(5) permits the namespace root to carry its own id as its parent id; such a table has a root, its self-edge is not an attachment, and `/data` under it resolves and freezes. A table in which every row hangs under another row still resolves nothing.
  - `a_thaw_of_a_completed_operation_is_not_held_by_a_superblock_it_never_froze`, `a_recovery_drain_over_the_same_topology_still_reports_the_hidden_superblock`, `a_tracked_thaw_reads_no_mount_table`, `an_operation_that_lost_a_worker_leaves_its_thaw_to_discovery`, `a_lost_thaw_worker_leaves_the_next_thaw_to_discovery`, `a_discovery_drain_uses_the_held_handle_of_the_same_superblock` (`handlers::fsfreeze`) — the thaw scope (§4.2): over `directory_overmount.txt` the operation that froze C through `/data/nested` thaws C through its held descriptor, opens nothing, removes the marker and admits the next cycle, twice over, although A is reachable by no name; a recovery drain over the same table (a restart) still reports A unreachable with the marker retained; a tracked thaw completes although the mount table cannot be read; an operation that lost a worker leaves a discovery drain, which reaches the uncertain target by its pathname; a thaw whose own blocking task is lost (the descriptors it took gone with it) leaves discovery to the next thaw too, so an empty descriptor list is never drained into a `Thawed`; under discovery a held handle serves its own superblock, matched by device. `handles_whose_drain_is_incomplete_are_held_for_the_next_thaw` and `unrecoverable_thaw_failure_rearms_watchdog` now expect the retry to drain the held target alone.
...
- **Implement (green):** `FreezePlan::restrict_to` indexes the mount table once (`MountTree`: children by `(parent id, mount point)`, the root being the mount at `/` whose parent is absent from the table or is the row itself, a self-parented row being left out of the child index), resolves each distinct requested name once as the kernel would (`MountTree::resolve`: from the root, cross into the child attached at the shortest prefix at or below the current mount point, repeat; the mount reached must be mounted at exactly that name), selects each target by the first name that led to its superblock and records it in `Target::requested`; `open_target` opens such a target on the requested name and on nothing else, so a name that does not lead to the selected superblock fails the operation instead of an alias standing in (before, any target carrying the name as its first mount point or an alias was selected, so a hidden name inflated the count; then the last row at the path was taken, which a `mount --move` defeats). After the commit, `frozen == 0 && held == 0 && unrecoverable.is_none()` (no worker outstanding by construction) takes `settle_nothing_frozen`: a tracked marker removal, then `conclude(Thawed, Ok(0))`; a removal failure other than absence concludes `Frozen` with `NothingFrozenMarkerRetained`. The count semantics of `freeze-list` are unchanged and now pinned; the controller rule is documented in §4.2 (one mount point per required superblock, `count == requested`), the validity half of the contract being T6.3. The thaw scope: `Driver::conclude` records on the context how the next thaw must find its targets (`ThawScope::Tracked` when the operation settles `Frozen` with no target uncertain, `Discovery` after a lost worker, drain or rollback, and on every `Thawed`); `run_thaw` reads no mount table under `Tracked` (`thaw_blocking` with no plan drains the held descriptors and removes the marker), takes the discovery path for a drain from `Thawed` or under `Discovery`, sets `Discovery` when its own blocking task is lost, and resets the scope when a thaw completes.
...
#### T6.3 — Freeze-validity contract and the reference snapshot controller (#43 §1)
- **Status:** done
- **Design:** §4.5 (new), §4.2 "Coverage", §4.4 (the lease), AC22, OQ-9.
- **Depends on:** T6.2
- **Files:** `docs/design.md` §4.5, `tests/controller_contract.rs` (new), `src/kernel/fake.rs` (`track_freeze_depth`, `reset_freeze_depths`, `tracked_frozen_superblocks`: the fake models the kernel's nesting per superblock, so a thaw count reflects what was frozen), `src/dispatch.rs` (`Context::abort_tasks`) and `src/freeze_op.rs` (`FreezeOp::abort_driver`): harness-only, `#[doc(hidden)]`, never called by the daemon; they end a dead instance's watchdog and driver task as `SIGKILL` would, so a crash modelled in one process leaves no ghost that could publish a state or thaw under the next cycle, and like the crash they leave the state machine where it was.
- **Tests first (red)** (`tests/controller_contract.rs`: the reference controller `Cycle` driven against the production dispatcher, coordinator and watchdog on paused time, one agent instance per "process" over a shared kernel and marker directory):
  - `a_cycle_inside_the_lease_is_quiesced_and_a_long_upload_does_not_revoke_it` — the normal case, and the verdict is not revisited when the upload outlives the lease.
  - `a_lease_that_expires_before_the_cut_is_rejected`, `a_lease_that_expires_during_the_cut_is_rejected` (heartbeats kept, the hard cap passed; the controller's budget stops the cut), `an_expiry_between_two_volume_snapshots_is_rejected` (the first volume alone is not a consistent set).
  - `a_freeze_reply_delayed_past_the_client_timeout_is_rejected` (the controller's own timeout; its thaw aborts the walk and it waits for `thawed`), `a_freeze_aborted_by_the_operation_deadline_is_rejected` (the agent gives up first).
  - `an_agent_restart_while_frozen_keeps_the_verdict_sound` (recovery mode over the new connection; the kernel kept the freezes), `a_guest_reboot_during_the_cut_is_rejected`, `a_crash_before_the_freeze_reply_is_rejected` (no count ever learned; the recovery drain over the restarted agent provably reaches the superblock the dead walk froze late: a success and an `EINVAL` there, one `EINVAL` on the one the walk never reached), `a_thaw_reply_past_the_budget_is_rejected_whatever_the_count` (step 5: the count alone cannot tell this cycle's lease from a restarted agent's re-armed cap).
  - `a_new_cycle_after_an_expired_one_quiesces_only_its_own_snapshots`, `a_foreign_refreeze_after_the_expiry_is_the_single_controller_assumptions_limit` (the boundary of the count, pinned; the agent's refusal of a second freeze while one is unresolved), `an_external_thaw_behind_a_frozen_status_is_rejected` (`frozen` is not a certificate), `a_request_that_is_not_fully_covered_is_rejected_before_any_cut`, `a_hidden_mount_point_cannot_stand_in_for_an_unprotected_requirement` (the external review's counterexample on `hidden_mount.txt`: `/data` over an overmount plus an unfreezable `/required` is a count of 1, rejected; with the old selection the hidden name made it 2 and the cycle was accepted), `a_mount_moved_over_a_newer_one_is_what_a_request_protects` (`moved_mount.txt`: A moved over B keeps its earlier row; the cycle protects A, opened on `/data`, B untouched, quiesced) `a_requirement_hidden_by_a_mount_over_its_ancestor_is_rejected` (`hidden_nested.txt`: 0 of 1) and `a_superblock_the_cycle_never_froze_cannot_hold_its_thaw` (`directory_overmount.txt`: two cycles over C while A stays hidden and never frozen; the thaw scope of §4.2 lets each finalise and admits the next).
  - `tracked_freeze_depth_grants_one_thaw_per_freeze_and_a_reboot_clears_it`, `tracked_freeze_depth_belongs_to_the_superblock_not_the_pathname` (`kernel::fake`); `a_dead_instances_walk_is_aborted_and_never_publishes` (`handlers::fsfreeze`, the unit-level pin of the two abort helpers for the mutants gate, which runs the library tests only).
...
- **Files:** `src/audit.rs` (`Router` over a writer thread and a bounded delivery queue with loss gaps (`LossGap`: one entry per position, a count per reason); `try_new`, `stderr() -> io::Result`, `unstarted`, `stderr_unstarted`, `start_writer`, `writer_started`, `settle`, `queued_bytes`, `queued_items`, `unreported_losses`, `with_queue_capacity`), `src/daemon.rs` (`init_logging` starts logging without the thread; `Startup::start_audit_writer`, step 5b, starts it after the drop; the exit path starts it to report a refusal), `tests/e2e/mod.rs` (`thread_statuses`), `src/handlers/shutdown.rs` (`AUDIT_DELIVERY_GRACE`), `tests/audit_freeze_window.rs`, `tests/privileged_e2e.rs`.
- **Tests first (red):**
  ...
  - `alternating_loss_reasons_never_grow_the_queue` (external review, finding 2: a hundred freeze windows overflowing behind a full queue leave three lines and one gap, every loss counted by reason), `a_gap_being_written_does_not_swallow_losses_recorded_meanwhile` (its follow-up: the writer owns the item in flight, a loss recorded while a gap is being written starts a new gap, reported + unreported == actual), `records_queued_before_the_writer_starts_are_delivered_once_it_does` (finding 3: logging before the drop queues, the writer started after it delivers; no grace waited for a thread that never started), `startup_order_is_config_marker_channel_caps_seccomp_runtime` (`tests/startup.rs`: "audit writer" is the step after "caps"), `privileged_every_thread_carries_the_dropped_ceiling` (`tests/privileged_e2e.rs`: every `/proc/<pid>/task/*/status` of the running daemon, the audit writer and the blocking-pool workers among them, shows uid 600, the final sets, the trimmed bounding set, `NoNewPrivs: 1` and `Seccomp: 2`).
...
- **Implement (green):** the sink is owned by a writer thread; `write` in `Normal` mode enqueues (or drops and counts at `SINK_QUEUE_CAPACITY`, 256 KiB), in `Ring` mode pushes to the ring; `flush_to_normal` moves the ring's loss record and lines to the queue and switches the mode without touching the sink; the writer parks while the mode is `Ring`; losses are `Item::Lost` markers in the FIFO so their record is delivered where the gap is; the last handle's drop waits a bounded grace for delivery (tests). No lock is held during a sink write. `guest-shutdown` waits at most `AUDIT_DELIVERY_GRACE` (2 s) for its record before `reboot(2)` and touches no descriptor itself. The ring's overflow is queued as a loss marker (zero bytes), never as a line a full queue could drop. `run_with` reports every failure after logging is up and, on every way out, flushes the ring and settles delivery under `audit::DELIVERY_GRACE` (tests: `a_failure_after_recovery_logging_started_still_reports_itself`, `leaving_delivers_the_queued_records_before_returning`, `shutdown_behind_a_full_stderr_pipe_still_reaches_the_kernel`, `a_ring_loss_is_reported_by_a_marker_even_when_the_queue_is_full`).
...
- **Tests first (red):**
  - `hardening_is_enforced_by_default_and_the_opt_out_is_unmistakable`, `a_disabled_filter_is_refused_under_enforced_hardening` (`config`).
...
- **Tests first (red):**
  - `the_authority_follows_the_feature_switches` (`config`); `a_profile_without_shutdown_gives_up_cap_sys_boot_everywhere` (`kernel::caps`: two finals, `CAP_SYS_BOOT` trimmed from the bounding set, never re-raised); `the_profile_carries_only_the_surfaces_the_configuration_enables` (`seccomp`: both targets, each switch alone; `FITRIM` only with trim).
  - `a_data_protection_profile_disables_the_optional_families_and_nothing_the_host_sends_re_enables_them` (`dispatch`: `CommandNotFound` "has been disabled" for the six optional commands in every state, no handler, freeze/thaw/status/ping unaffected, `guest-info` agrees).
  - `data_protection_example_parses_and_keeps_only_the_snapshot_authority` (`tests/packaging.rs`).
  - `a_cycle_under_the_data_protection_profile_needs_no_discovery` (`tests/controller_contract.rs`): the reference controller under the profile's switches, `guest-get-fsinfo` answering "disabled", the required mount points provisioned rather than discovered; a quiesced cycle, and a wrongly provisioned mount point rejected (§4.5 step 1, §5.9 "The controller under this profile"; external review).
...
- **Tests first (red):**
  ...
  - `a_flood_of_denied_and_malformed_frames_is_bounded_by_the_audit_queue` (`tests/audit_freeze_window.rs`: 4 000 denied and malformed frames with the sink blocked throughout; the excess is dropped and counted, a ping still answers, and once the sink moves the records delivered plus the records the loss records account for are exactly the 4 001 frames: one record per frame under the flood).
  - The remaining rows of §5.10 are pinned by the tests named in their tasks (T1.2, T1.3, T1.4, T1.7, T2.5, T4.1, T6.1, T6.4; the audit queue's entry bound by `alternating_loss_reasons_never_grow_the_queue`, external review).
- **Implement (green):** the two reply bounds through `bounded_reply` (serialise, add the longest response envelope `proto::MAX_RESPONSE_ENVELOPE_BYTES` so the bound holds for the line as sent, compare, refuse with the size and the bound; test `the_reply_bound_covers_the_whole_line_envelope_included`); everything else was already bounded and is now inventoried in §5.10 with its behaviour past the bound and the failure model (recovery independent of the host; liveness assumes the runtime is scheduled and the kernel calls progress; no reply can be delivered to a peer that refuses it). Duplicate recovery requests are serialised behind the drain in progress rather than coalesced: each is answered from the kernel's own state at one `FITHAW` per drained target, the session's places bound how many wait, and recovery capacity (the watchdog, the coordinator) is independent of the queue.
