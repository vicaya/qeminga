...
//! sent while the freeze reply is pending reaches the operation, the
//! frozen-safe controls are served while a recovery drain is blocked,
//! replies keep request order (requests with and without `id`, the
//! delimited sync), repeated controls are bounded, and a lost connection
//! leaves the operation owned.
#![forbid(unsafe_code)]
...
use qeminga::channel::{self, MAX_IN_FLIGHT, SessionEnd, run_session_until};
...
async fn repeated_controls_while_a_reply_is_pending_are_bounded_and_answered_in_order() {
    // Twenty status requests behind a pending freeze reply: at most
    // MAX_IN_FLIGHT are handled at once (the rest wait unread), no ioctl
    // or worker is added, and every reply follows the freeze reply in
    // order.
    let mut rig = rig();
    let (gate, _release) = rig.freeze_blocked_at_b().await;
    ...
    assert!(
        rig.ctx.handler_calls() <= (MAX_IN_FLIGHT as u64) + 1,
        "handled {} commands with the freeze pending",
